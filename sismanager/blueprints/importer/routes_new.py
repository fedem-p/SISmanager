        ...
        for file in files:
            ...
            try:
                ...
                uploaded_files.append(
                    {"file_id": file_metadata["id"], "success": True, **file_metadata}
                )
...
def process_file(file_id: str):
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "file_id": file_id,
                "processed_rows": (
                    len(importer.rows) if hasattr(importer, "rows") else 0
                ),
            }
        )
        ...
        return (
            jsonify({"success": False, "message": f"Processing failed: {str(e)}"}),
            500,
        )
...
def remove_duplicates(file_id: str):
    ...
    try:
        ...
        if not file_info or file_info["status"] != "processed":
            return (
                jsonify(
                    {"success": False, "message": "File not found or not processed"}
                ),
                404,
            )
        ...
        return (
            jsonify({"success": False, "message": f"Deduplication failed: {str(e)}"}),
            500,
        )
...
def export_file(file_id: str):
    ...
    try:
        ...
        if not download_path:
            return (
                jsonify(
                    {"success": False, "message": "Failed to create download file"}
                ),
                500,
            )
...
def get_all_files_status():
    ...
    try:
        ...
        return jsonify({"success": True, "files": files_status, "stats": stats})
